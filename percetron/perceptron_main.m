function [outputArg1,outputArg2] = perceptron_main(data,w_para,parameters)
%PERCEPTRON_MAIN この関数の概要をここに記述
%   詳細説明をここに記述
...
for time_cv=1: cross_vaild
    ...
    for i_class1=1:(num_class-1)
        for i_class2=(i_class1+1):num_class
            ...
            model{i_class1,i_class2}=per_training(Train_sets,Train_labels);
            disp(['training times is ',num2str(model{i_class1,i_class2}.training_iter)]);
            %w=w_para;
        end
    end
    ...
    for test_class1=1:(num_class-1)
        for test_class2=(test_class1+1):num_class
            predict_label=per_testing(model{test_class1,test_class2},data{time_cv,1}(:,1:end-1),test_class1,test_class2);
    ...
    for test_class1=1:(num_class-1)
        for test_class2=(test_class1+1):num_class
            predict_label=per_testing(model{test_class1,test_class2},data{time_cv,2}(:,1:end-1),test_class1,test_class2);
